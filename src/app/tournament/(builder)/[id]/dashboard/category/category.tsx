...
import {
  ...
  updateCategory,
} from "@/app/service/tournament/categoryService";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
interface props {
  tournamentId: string;
  categories: TournamentCategoryRequest[];
}
...
const TournamentCategory = ({ tournamentId, categories }: props) => {
  const [isAddEvent, setIsAddEvent] = useState(false);
  const [isEditEvent, setIsEditEvent] = useState(false);
  const [ageGroupStr, setAgeGroupStr] = useState("");
  const [ageGroup, setAgeGroup] = useState<Option[]>([]);
  const [previewList, setPreviewList] = useState<ICategory[]>(categories || []);
  ...
  const onEditCategory = (cat: ICategory) => {
  ...
  const onSubmit = async (data: z.output<typeof formSchema>) => {
    ...
    try {
      ...
      if (isEditEvent) {
        const catObj = categories.find((cat) => cat.id === parsedData.id);
        result = await updateCategory(payload, catObj?.createdAt || "");
        ...
        result = await setCategory(payload);
